...
export default class AppViewModel {
  ...
  constructor(managers: Managers) {
    ...
    reaction(
      () =>
        this.webrtcSDK.meetingService.closedCaptioningService
          ?.closedCaptioningState,
      (closedCaptioningState: ClosedCaptioningState) => {
  ...
  @computed get videoMessage(): string {
    if (this.webrtcSDK.meetingService.contentService?.receivingContentShare) {
      ...
      this.webrtcSDK.meetingService.videoState ==
      VideoState.INACTIVE_NEEDS_MODERATOR
    ) {
      return 'Waiting for the moderator'
    } else if (
      this.webrtcSDK.meetingService.videoState ==
      VideoState.INACTIVE_ONLY_PARTICIPANT
    ) {
  ...
  @computed get showCaptionText(): boolean {
    return (
      this.webrtcSDK.meetingService.closedCaptioningService
        ?.isClosedCaptioningAvailable && this.captionText?.length > 0
    )
  }
  ...
  @computed get captionText(): string {
    ...
    if (
      this.webrtcSDK.meetingService.closedCaptioningService
        ?.closedCaptioningState === ClosedCaptioningState.CONNECTING &&
      !this.hasLoadingTranscriptShown
    ) {
      return (messageToUI = 'loading')
    } else {
      // Trimming the string to the max length, which we can display in the screen.
      messageToUI =
        this.webrtcSDK.meetingService.closedCaptioningService?.closedCaptionText
      return messageToUI?.length > CC_DISPLAY_CHAR_LIMIT
        ? `${messageToUI.slice(0, CC_DISPLAY_CHAR_LIMIT)}...`
        : messageToUI
    }
  }
  ...
  @computed get screenShareOnly() {
    return (
      this.webrtcSDK.meetingService.connectionMode ===
      ConnectionMode.ScreenShareOnly
    )
  }
}
