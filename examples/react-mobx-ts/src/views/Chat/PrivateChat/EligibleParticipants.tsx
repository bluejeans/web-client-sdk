...
import { ParticipantListContainer, ParticipantListItem, ParticipantName } from "./styles/PrivateChatComponent"
...
export default class EligibleParticipants extends Component<Props> {
    ...
    renderParticipant(participant: Participant, n): JSX.Element {
        let unreadMessagesCount = this.viewModel.unreadPrivateMessagesCount.get(participant.participantGuid);
        let shouldShowUnreadMessagesCount = unreadMessagesCount > 0;
        return (
        <ParticipantListItem key={n.toString()} onClick={()=>this.viewModel.showChatHistory(participant)}>
