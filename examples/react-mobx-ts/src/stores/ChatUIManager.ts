...
export default class ChatUIManager {
  ...
  @observable privateChatViewToShow: PRIVATECHATVIEW =
    PRIVATECHATVIEW.PARTICIPANTS
  ...
  @computed get isPrivateChatEnabled(): boolean {
    return this.webrtcSDK.meetingService.privateChatService
  ...
  @computed get unreadPrivateMessagesCountForParticipant(): Map<
    string,
    number
  > {
    return this.webrtcSDK.meetingService?.privateChatService
  ...
  @computed get messages(): ChatMessage[] {
    return this.chatPanelToShow === CHATPANEL.PUBLIC
      ? this.webrtcSDK.meetingService?.publicChatService?.chatHistory
      : this.webrtcSDK.meetingService?.privateChatService?.chatHistoryByParticipant.get(
          this.selectedParticipant?.participantGuid,
        )
  }
  ...
  sendPrivateMessage(message: string, participantGuid: string) {
    this.webrtcSDK.meetingService?.privateChatService?.sendMessage(
      message,
      participantGuid,
    )
  }
  ...
  clearUnreadPrivateMessageCount(participantGuid: string): void {
    this.webrtcSDK.meetingService?.privateChatService?.clearUnreadMessageCountForParticipant(
      participantGuid,
    )
  }
  ...
  clearChatCount(): void {
    if (this.chatPanelToShow === CHATPANEL.PUBLIC) {
      ...
      this.clearUnreadPrivateMessageCount(
        this.selectedParticipant?.participantGuid,
      )
    }
  }
