...
import { BJNWebClientSDK, Participant, videoStream, StreamQuality, StreamPriority, VideoLayout } from "@bluejeans/web-client-sdk";
...
export default class CustomLayoutManager {
    ...
    constructor(webrtcSDK : BJNWebClientSDK) {
        ...
        reaction(() => this.pinnedParticipant?.participantGuid, () => {
    ...
    setVideoStreamConfiguration() {
        ...
        this.webrtcSDK.meetingService.videoStreamService?.setVideoStreamConfiguration(config)
    }

    @computed get getVideoStreamConfigForFirstModerator() {
        if (!this.firstModerator?.isVideoMuted) {
            return { participantGuid: this.firstModerator?.participantGuid, streamQuality: StreamQuality.r720p_30fps, streamPriority: StreamPriority.HIGH }
        }
    }

    @computed get getVideoStreamConfigPinnedParticipant() {
        if (this.pinnedParticipant && !this.pinnedParticipant.isVideoMuted) {
            return { participantGuid: this.pinnedParticipant?.participantGuid, streamQuality: StreamQuality.r720p_30fps, streamPriority: StreamPriority.HIGH }
        }
    }
    get getVideoStreamConfiguration() {
        return [
            { streamQuality: StreamQuality.r180p_30fps, streamPriority: StreamPriority.MEDIUM },
            { streamQuality: StreamQuality.r180p_30fps, streamPriority: StreamPriority.MEDIUM },
            { streamQuality: StreamQuality.r180p_30fps, streamPriority: StreamPriority.MEDIUM },
            { streamQuality: StreamQuality.r180p_30fps, streamPriority: StreamPriority.MEDIUM },
            { streamQuality: StreamQuality.r180p_30fps, streamPriority: StreamPriority.MEDIUM },
    ...
    @computed get firstModerator() {
        if (this.webrtcSDK.meetingService.videoLayout === VideoLayout.CUSTOM) {
            let arrWithoutSelfParticipant = this.webrtcSDK.meetingService.participantService?.participants.slice(1)
            let indexOfFirstModerator = arrWithoutSelfParticipant?.findIndex(i => i.isModerator)
            if (arrWithoutSelfParticipant?.[indexOfFirstModerator]?.participantGuid === this.pinnedParticipant?.participantGuid) {
    ...
    @computed get videoStreamsMap(): Map<string, videoStream> {
        let videoStreamsArr = this.webrtcSDK.meetingService.videoStreamService?.videoStreams?.map(obj => {
    ...
    @computed get pinnedParticipant(){
       ...
       return this.webrtcSDK.meetingService.participantService?.participants?.find( participant => participant.participantGuid === pinnedParticipant);
    }
}